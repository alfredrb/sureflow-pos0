...
const OP_RANGES = { manager: [1000, 1999], loss_prevention: [1000, 1999], csm: [2000, 2999], cashier: [3000, 3999], technician: [9000, 9999] };
const genOperatorIdForRole = (role) => {
  const r = OP_RANGES[role];
  if (!r) return "";
  return String(r[0] + Math.floor(Math.random() * (r[1] - r[0] + 1)));
};
...
export default function AdminEmployeeCreation() {
  ...
  const onRoleChange = (v) => {
    setOp(p => ({ ...p, role: v, pos_access: v === "loss_prevention" ? false : true, operator_id: genOperatorIdForRole(v), pin: genPin() }));
  };
  const regenerate = () => setOp(p => ({ ...p, operator_id: genOperatorIdForRole(p.role), pin: genPin() }));
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex gap-2">
              <Input value={op.operator_id} onChange={e => setOp(p => ({ ...p, operator_id: e.target.value }))} placeholder="generated by role" />
              <Button type="button" variant="outline" onClick={regenerate} className="shrink-0">Regenerate</Button>
            </div>
            <p className="text-xs text-gray-400 mt-1">Auto-generated by role: 1xxx Manager/LP · 2xxx CSM · 3xxx Cashier · 9xxx Tech.</p>
          </div>
          <div>
