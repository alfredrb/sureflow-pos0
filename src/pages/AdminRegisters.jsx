...
import DrawerTransportSection from "@/components/registers/DrawerTransportSection";
import { drawerTransportLabel, DRAWER_BRIDGE_PORT } from "@/lib/drawerProfiles";
import PoleDisplaySection from "@/components/registers/PoleDisplaySection";
import CustomerMonitorSection from "@/components/registers/CustomerMonitorSection";
import SelfCheckoutSection from "@/components/registers/SelfCheckoutSection";
...
const emptyReg = { register_id: "", name: "", location: "", status: "offline", ip_address: "", subnet_mask: "255.255.255.0", gateway: "", assigned_operator: "", cash_limit: 5000, feature_returns: false, feature_customer_service: false, feature_exchange: false, feature_self_checkout: false, feature_attendant: false, attendant_register_id: "", printer_status: "unknown", scanner_status: "unknown", cash_drawer_status: "unknown", printer_model: "", printer_ip: "", printer_transport: "ethernet", printer_fallback_ip: "", scanner_model: "", cash_drawer_model: "", printer_serial: "", scanner_serial: "", cash_drawer_serial: "", drawer_transport: "printer_dk", drawer_bridge_ip: "", drawer_bridge_port: DRAWER_BRIDGE_PORT, drawer_model: "", terminal_model: "", terminal_serial: "", mac_address: "", boot_profile: "local_disk", keyboard_model: "", scanner_interface: "usb_hid", pxe_vlan: "", backend_vlan: "", pinpad_model: "", pinpad_ip: "", pinpad_serial: "", pole_display_model: "", pole_display_ip: "", pole_display_serial: "", customer_monitor_enabled: false, customer_monitor_resolution: "1920x1080", customer_monitor_orientation: "landscape", customer_monitor_serial: "", store_id: "" };

const AUDIT_FIELDS = ["register_id", "name", "location", "status", "ip_address", "subnet_mask", "gateway", "assigned_operator", "cash_limit", "feature_returns", "feature_customer_service", "feature_exchange", "feature_self_checkout", "feature_attendant", "attendant_register_id", "printer_model", "printer_ip", "printer_transport", "printer_fallback_ip", "printer_serial", "printer_status", "scanner_model", "scanner_serial", "scanner_status", "cash_drawer_model", "cash_drawer_serial", "cash_drawer_status", "drawer_transport", "drawer_bridge_ip", "drawer_bridge_port", "drawer_model", "terminal_model", "terminal_serial", "mac_address", "boot_profile", "keyboard_model", "scanner_interface", "pxe_vlan", "backend_vlan", "pinpad_model", "pinpad_ip", "pinpad_serial", "pole_display_model", "pole_display_ip", "pole_display_serial", "customer_monitor_enabled", "customer_monitor_resolution", "customer_monitor_orientation", "customer_monitor_serial", "store_id"];
...
export default function AdminRegisters() {
  ...
  const openEdit = (r) => {
    setEditing(r);
    setForm({ register_id: r.register_id, name: r.name, location: r.location || "", status: r.status, ip_address: r.ip_address || "", subnet_mask: r.subnet_mask || "255.255.255.0", gateway: r.gateway || "", assigned_operator: r.assigned_operator || "", cash_limit: r.cash_limit || 5000, feature_returns: r.feature_returns || false, feature_customer_service: r.feature_customer_service || false, feature_exchange: r.feature_exchange || false, feature_self_checkout: r.feature_self_checkout || false, feature_attendant: r.feature_attendant || false, attendant_register_id: r.attendant_register_id || "", printer_status: r.printer_status || "unknown", scanner_status: r.scanner_status || "unknown", cash_drawer_status: r.cash_drawer_status || "unknown", printer_model: r.printer_model || "", printer_ip: r.printer_ip || "", printer_transport: r.printer_transport || "ethernet", printer_fallback_ip: r.printer_fallback_ip || "", scanner_model: r.scanner_model || "", cash_drawer_model: r.cash_drawer_model || "", printer_serial: r.printer_serial || "", scanner_serial: r.scanner_serial || "", cash_drawer_serial: r.cash_drawer_serial || "", drawer_transport: r.drawer_transport || "printer_dk", drawer_bridge_ip: r.drawer_bridge_ip || "", drawer_bridge_port: r.drawer_bridge_port || DRAWER_BRIDGE_PORT, drawer_model: r.drawer_model || "", terminal_model: r.terminal_model || "", terminal_serial: r.terminal_serial || "", mac_address: r.mac_address || "", boot_profile: r.boot_profile || "local_disk", keyboard_model: r.keyboard_model || "", scanner_interface: r.scanner_interface || "usb_hid", pxe_vlan: r.pxe_vlan || "", backend_vlan: r.backend_vlan || "", pinpad_model: r.pinpad_model || "", pinpad_ip: r.pinpad_ip || "", pinpad_serial: r.pinpad_serial || "", pole_display_model: r.pole_display_model || "", pole_display_ip: r.pole_display_ip || "", pole_display_serial: r.pole_display_serial || "", customer_monitor_enabled: r.customer_monitor_enabled || false, customer_monitor_resolution: r.customer_monitor_resolution || "1920x1080", customer_monitor_orientation: r.customer_monitor_orientation || "landscape", customer_monitor_serial: r.customer_monitor_serial || "", store_id: r.store_id || "" });
    setDialogOpen(true);
  };
        ...
        {scopedRegisters.map(r => (
          <div key={r.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-1.5 text-sm mb-4">
              ...
              <div className="flex justify-between"><span className="text-gray-400">Drawer</span><span className="text-gray-700 text-xs">{r.drawer_transport === "usb_direct" ? "USB (reserved)" : "Printer DK (SDL)"}</span></div>
              ...
              <div className="flex justify-between"><span className="text-gray-400">Customer Monitor</span><span className="text-gray-700 text-xs">{r.customer_monitor_enabled ? `${r.customer_monitor_resolution || "1920x1080"} ${r.customer_monitor_orientation || "landscape"}` : "—"}</span></div>
              <div className="flex justify-between"><span className="text-gray-400">Self-Checkout</span><span className="text-gray-700 text-xs">{r.feature_self_checkout ? `Yes${r.attendant_register_id ? ` · att. ${r.attendant_register_id}` : ""}` : r.feature_attendant ? "Attendant station" : "—"}</span></div>
