...
export default function VersionButton({ collapsed, canManage, adminOperator }) {
  ...
  const handleSave = async () => {
    if (!form.version.trim()) {
      toast({ title: "Version number is required", variant: "destructive" });
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        {!collapsed && current && <span className="ml-auto text-[10px] text-indigo-300/70 font-mono">v{current.version}</span>}
      </button>
                    ...
                    return (
                      <div key={idx} className="flex items-start gap-2">
                        ...
                        {form.changes.length > 1 && (
                          <button type="button" onClick={() => removeChangeRow(idx)} className="p-2 text-gray-400 hover:text-red-500">
                            <X className="w-4 h-4" />
                          </button>
                        )}
                      </div>
