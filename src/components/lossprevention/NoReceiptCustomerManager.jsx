...
import { Search, Plus, ShieldX, ShieldCheck, Eye } from "lucide-react";
...
export default function NoReceiptCustomerManager() {
  ...
  const saveDisable = async () => {
    ...
    try {
      ...
      await logAuditEvent({ action: "Disabled No-Receipt Customer", category: "permissions", description: `Disabled customer ${disableTarget.customer_id} from no-receipt returns. Reason: ${disableReason || "n/a"}`, page: "/admin/loss-prevention" });
  ...
  const enableCustomer = async (c) => {
    ...
    await logAuditEvent({ action: "Enabled No-Receipt Customer", category: "permissions", description: `Re-enabled customer ${c.customer_id} for no-receipt returns`, page: "/admin/loss-prevention" });
  ...
  const createPreemptive = async () => {
    ...
    await logAuditEvent({ action: "Added No-Receipt Customer", category: "permissions", description: `Added customer ${newCust.customer_id} (disabled: ${newCust.disabled})`, page: "/admin/loss-prevention" });
