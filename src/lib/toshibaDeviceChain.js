...
export const DEVICE_CHAIN_NOTES = [
  ...
  "TELLING A LANE FROM THE CONTROLLER: do NOT go by the shell prompt. Every diskless lane boots with the controller's hostname baked into the image, so a lane at 10.0.40.105 also presents as sfc-001-a and an SSH session on a lane looks identical to one on the controller. Confirm which box you are on with 'ip -4 addr' (a lane is on the PXE VLAN) or 'cat /proc/cmdline' (a lane carries nfsroot= and sureflow.register_id=).",
  "Verify on a lane that has been REBOOTED ONTO A FRESHLY REBUILT legacy image. The module is baked in at build time only — running the builder is not enough, and neither is rebooting a lane onto the old root. Until both have happened, lsmod is empty and 0000:11:06.0 still reports no driver, which is the pre-fix state and not a failure of the fix.",
  "On such a lane: lsmod | grep aipdcs4, re-run the sysfs PCI loop expecting driver=aipdcs4 on 0000:11:06.0 instead of blank, then ls -l /dev/aipdcs*. If the module loads but no node appears, check dmesg | grep -i aipdcs for its probe output before assuming the node name is wrong.",
];