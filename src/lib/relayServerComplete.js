// The COMPLETE relay server.js — every module the fleet now runs, mounted in the one
// order that works. Use this instead of hand-patching when a relay is behind by
// several features, or when a hand-patched file has drifted.
//
// MOUNT ORDER IS THE WHOLE GAME. Express matches top to bottom, so:
//   1. json body parser + CORS
//   2. open POS routes (/status, /kiosk, /api/*, peripherals)
//   3. token-gated privileged routes (/proxmox, /ops, /lane/reboot)
//   4. static POS build
//   5. SPA catch-all           <-- ANYTHING BELOW THIS IS UNREACHABLE
//   6. error handler
// A route added under the catch-all returns the POS's index.html instead of JSON,
// which looks exactly like "the endpoint doesn't exist".

export const RELAY_SERVER_COMPLETE_CODE = `// server.js — SureFlow Local Relay (complete: phase 3 + cheque + pinpad + pole + lane reboot)
...
curl -s -o /dev/null -w '%{http_code}\\n' -X POST http://localhost:3000/lane/reboot \\
  -H 'Content-Type: application/json' -d '{"register_id":"REG-005"}'
`;