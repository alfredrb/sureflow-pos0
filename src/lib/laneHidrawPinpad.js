...
export const HID_PINPAD_NOTES = [
  ...
  "TRANSPORT IS NOW PROVEN CORRECT AND THE PAD IS STILL SILENT. On REG-005 the bridge wrote '02 02 30 38 2e 30 03 15' + zero padding as a single 32-byte OUTPUT report with no write error, and the pad answered nothing — and it still emits no INPUT report even for keypresses. Every software variable is now exhausted, so the fault is the PAD'S OWN INTERFACE CONFIGURATION: its COM setting was put on ETHERNET (menu 2-6-3-4) back when it was believed to be a network pad. RBA bound to the Ethernet interface neither listens on nor transmits over USB, which explains silence in BOTH directions exactly. Set the pad's communication interface to USB / USB-HID in its setup menu and power-cycle before doing anything else. DO NOT set an iSC250 to Ethernet on a USB-attached lane.",
